...
export const StreamContract = ({ amIAStreamedBuilder }: StreamContractProps) => {
  ...
  const { data: streamContract } = useDeployedContractInfo("StreamContract");

  const { data: owner } = useScaffoldContractRead({
    contractName: "StreamContract",
    functionName: "owner",
  });
  ...
  const { writeAsync: doWithdraw } = useScaffoldContractWrite({
    ...
    args: [ethers.utils.parseEther(amount || "0").toBigInt(), reason],
  });
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between px-8 text-xl pb-8">
        <div className="flex flex-col items-start">
          ...
          <Balance address={streamContract?.address} className="text-3xl h-14 p-6 rounded-md mb-6" />
